...
Library::Library(GraphicsDevice* gDevice, InputDevice* iDevice, PhysicsDevice* pDevice, AudioDevice* aDevice): 
	...
	for (tinyxml2::XMLElement* element = root->FirstChildElement("Sound"); element; element = element->NextSiblingElement("SFX")) {
	...
	for (tinyxml2::XMLElement* element = root->FirstChildElement("Song"); element; element = element->NextSiblingElement("Song")) {
